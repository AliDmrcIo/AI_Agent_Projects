"""
HATA: !!!! BU KOD ÇALIŞMAYACAK ÇÜNKÜ BU PLANLAMA KISIMLARININ VERSİYONLARI UYMUYOR !!!!!!

plan and execute multiple steps in a single agent call
1. plan yap (plan)
2. adım adım uygula (execute)

senaryo: bir öğrencinin sınav notunu hesapla sonucu değerlendir

1) plan: sınavları topla, ortalamayı hesapla, sonucu değerlendir(yorum yap)
2) execute: plani toolar ile adım adım uygula
...
from langchain_google_genai import ChatGoogleGenerativeAI # chat tabanlı openai modeli
from langchain.agents import Tool # agent ve tool kullanımı için gerekli sınıflar
from langchain.experimental.plan_and_execute import PlanAndExecute, load_agent_executor, load_chat_planner
from langchain.tools import tool # langchain araçlarını içeri aktar
...
gemini_api_key = os.getenv("GEMINI_API_KEY") # .env dosyasından OpenAI API anahtarını al
...
tools = [notlari_topla_tool, ortalama_hesapla_tool, degerlendir_tool]

planner = load_chat_planner(llm) # planlama işlemini llm yapacak
...
hedef = "90, 80, 75 notlarım var. Sınav notumu hesapla ve sonucu değerlendir."
